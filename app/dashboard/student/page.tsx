...
export default function TeacherDashboard() {
  ...
  const filteredStudentSummaries = useMemo(() => {
    if (reportStudent === "all") return studentSummaries;
    return studentSummaries.filter((s) => s.name === reportStudent);
  }, [studentSummaries, reportStudent]);
  ...
  const handleExportCSV = () => {
    if (reportView === "siswa") {
      ...
      const rows = filteredStudentSummaries.map((s) => [
        s.name,
        s.classCode,
      ...
      downloadCSV(
        headers,
        rows,
        `laporan-persiswa-${reportStudent === "all" ? "semua" : reportStudent}-${new Date().toISOString().slice(0, 10)}.csv`
      );
    } else {
      const headers = [
        "Nama Siswa",
      ...
      <div className="relative max-w-6xl mx-auto print:hidden">
        ...
        {activeTab === "laporan" && (
          <div className="bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-sm shadow-emerald-900/5 border border-white space-y-4">
            ...
            <p className="text-xs text-emerald-700/60">
              {reportView === "siswa"
                ? `Menampilkan ${filteredStudentSummaries.length} siswa${reportStudent !== "all" ? ` · ${reportStudent}` : ""}.`
