...
function getWeatherInfo(code: number): { label: string; Icon: typeof Sun; color: string } {
  if (code === 0 || code === 1) return { label: "Cerah", Icon: Sun, color: "text-amber-300" };
  if (code === 2) return { label: "Cerah Berawan", Icon: CloudSun, color: "text-sky-200" };
  if (code === 3) return { label: "Berawan", Icon: Cloud, color: "text-slate-200" };
  if (code >= 60 && code <= 69) return { label: "Hujan", Icon: CloudRain, color: "text-sky-200" };
  if (code >= 95) return { label: "Badai Petir", Icon: CloudLightning, color: "text-amber-200" };
  return { label: "Berawan", Icon: Cloud, color: "text-slate-200" };
}
                  ...
                  <>
                    ...
                    <div className="flex shrink-0 items-center gap-2.5 text-right">
                      <div>
                        ...
                        <p className="mt-1 text-[10px] text-emerald-50/70">Kelembapan {weather.humidity}%</p>
                      </div>
